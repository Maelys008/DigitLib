      ...
      <div className="min-h-screen bg-[#F9F9F9] dark:bg-gray-900">
        ...
        <div className="px-4 py-6 space-y-2">
          {cards.map((card) => (
            <div 
              key={card.id}
              className="flex items-center gap-4 p-4 bg-[#F2F2F7] dark:bg-gray-800 rounded-xl transition-active active:opacity-70"
            >
              <div className="w-8 h-8 flex items-center justify-center">
                {card.type === 'google' && <span className="font-bold text-xl dark:text-white">G</span>}
                {card.type === 'apple' && (
                  <svg className="w-6 h-6 fill-current dark:text-white" viewBox="0 0 384 512">
          ...
          <button
            ...
            className="w-full flex items-center justify-between p-4 bg-[#F2F2F7] dark:bg-gray-800 rounded-xl"
          >
