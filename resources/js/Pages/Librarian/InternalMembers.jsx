...
export default function InternalMembers() {
    ...
    const [formData, setFormData] = useState({ email: "", role_id: 3 }); // 🔥 3 = Assistant
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [error, setError] = useState("");

    // 🔥 Rôle Assistant uniquement
    const roles = [
        {
            id: 3,
            name: "Assistant",
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            if (activeLibrary) {
                console.log(
                    "📚 InternalMembers - Utilisation de la bibliothèque active:",
                    activeLibrary.name,
                );
                ...
                const ownedLib = libraries.find(
                    (lib) => lib.administrator_id === user.id,
                );
                if (ownedLib) {
                    setLibrary(ownedLib);
    ...
    const loadMembers = async (libraryId) => {
        try {
            ...
            // 🔥 Filtrer pour ne montrer que les assistants (role_id = 3)
            const assistants = response.filter(
                (member) => member.roles?.[0]?.id === 3,
            );
    ...
    const handleAddMember = async (e) => {
        ...
        const result = await api.addInternalMember({
            ...
            role_id: 3, // 🔥 Force le rôle Assistant
        });
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 sticky top-0 z-10">
                <div className="flex items-center justify-between">
                    ...
                    >
                        <Plus className="w-4 h-4" /> Ajouter un assistant
                    </button>
                </div>
            </div>
            ...
            <div className="p-6">
                ...
                {members.length === 0 && (
                    <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl">
                        ...
                        <button
                            onClick={() => setShowAddModal(true)}
                            className="mt-3 text-purple-600 dark:text-purple-400 text-sm"
                        >
                    ...
                    <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 rounded-t-3xl z-50 animate-slide-up max-h-[85vh] overflow-y-auto">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-600 rounded-xl text-gray-700 dark:text-gray-300">
                                    Assistant (gestion des livres et emprunts)
                                </div>
                            </div>
