...
class LoanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($user, $book, $library) {
            ...
            // 6. Vérifier si l'utilisateur a une réservation prioritaire
            $existingReservation = Reservation::where('user_id', $user->id)
                ->where('book_id', $book->id)
                ->whereIn('status', ['active', 'notified'])
                ->first();

            // 7. Chercher une copie disponible
            $copy = $book->copies()
    ...
    /**
     * Mettre à jour un prêt (retour avec pénalité)
     */
        ...
        return DB::transaction(function () use ($loan, $request) {
            ...
            if ($isLate || $request->has('amount')) {
                ...
                Notification::create([
                    'user_id' => $user->id,
                    'type' => 'penalty',
                    'message' => "Une pénalité de " . ($request->amount) . " FCFA a été ajoutée suite à : " . ($request->reason),
                    'object_type' => 'loan',
                    'object' => $loan->id,
                    'date_sent' => now(),
                ]);
    ...
    /**
     * Rafraîchir le badge de l'utilisateur
     */
